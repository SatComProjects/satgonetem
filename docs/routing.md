...
self.topology.status              # True once TopologyManager has loaded (SimulationManager is live)
self.topology.gonetem_is_on       # True once containers are running (GoNetEm is active)
```
...
class LatencyDaemon(RoutingDaemon):
    ...
    def remove(self, node=None, max_workers: int = 4) -> None:
        ...
        for n in targets:
            if n.container:
                n.execute_command(["ip", "route", "flush", "table", "main"])
