...
`HILManager` works **alongside** any `NetworkLauncher` (DirectLauncher or
GoNetEmLauncher). The launcher continues to manage all satellites, ISLs, and
...
TopologyManager
  |
  +-- NetworkLauncher (DirectLauncher or GoNetEmLauncher)
...
underlying DirectLauncher and GoNetEmLauncher already require root for
...
automatically. The launcher (DirectLauncher or GoNetEmLauncher) is unaware of
...
no reference to the launcher and works with both DirectLauncher and
GoNetEmLauncher as long as `container_pid` is populated on satellite nodes.
