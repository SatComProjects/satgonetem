...
The `satgonetem.traffic` package provides three tools for sending traffic between nodes in a running topology: ping, iperf3, and hping3. Each tool follows the same pattern: a config dataclass, a results class, and a non-blocking flow class.
...
The primary use case is replaying a captured trace where each flow has a realistic start offset:
